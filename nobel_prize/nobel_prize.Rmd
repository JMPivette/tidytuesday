...
fem_cumul_freq <- nobel_winners %>% 
  ...
  count(gender) %>% 
  ungroup() %>% 
  complete(prize_year, category, gender, fill = list(n = 0)) %>% 
  spread(key = gender, value = n, fill = 0) %>% 
  group_by(category) %>% 
  mutate(fem_cumulative = cumsum(Female),
         male_cumulative = cumsum(Male),
         cum_freq = ifelse(fem_cumulative + male_cumulative == 0,
                           0,
                           fem_cumulative/(fem_cumulative + male_cumulative))) %>% 
...
fem_cumul_freq %>% 
  ggplot(aes(x = prize_year, y = cum_freq, color = category)) + 
    ...
    labs(x = "Prize year", y = "Cumulative female winners", caption = 'Data source : Harvard Dataverse | plot by @jpivette')
...
p <- fem_cumul_freq %>% 
  group_by(prize_year) %>% 
  mutate(rank = row_number(cum_freq)) %>% 
